...
import com.jeemoo.common.utils.TreeBuildUtils;
import com.jeemoo.system.domain.SysRoleMenu;
import com.jeemoo.system.domain.vo.AiCompanyVo;
import com.jeemoo.system.mapper.SysMenuMapper;
import com.jeemoo.system.mapper.SysRoleMapper;
import com.jeemoo.system.mapper.SysRoleMenuMapper;
...
public class SysMenuServiceImpl implements ISysMenuService {
    ...
    @Override
    public List<SysMenu> selectCompanyMenuList(LoginUser loginUser, AiCompanyVo companyVo) {
        List<SysMenu> menuList = null;
        // 管理员显示所有菜单信息
        if (Objects.equals(companyVo.getUserId(), loginUser.getUserId())) {
            menuList = baseMapper.selectList(new LambdaQueryWrapper<SysMenu>()
                            .eq(SysMenu::getIsCompany,1)
                    .orderByAsc(SysMenu::getParentId)
                    .orderByAsc(SysMenu::getOrderNum));
        } else {
            QueryWrapper<SysMenu> wrapper = Wrappers.query();
            wrapper.eq("sur.user_id", loginUser.getUserId())
                    .eq("m.is_company",1)
                    .orderByAsc("m.parent_id")
                    .orderByAsc("m.order_num");
            menuList = baseMapper.selectMenuListByUserId(wrapper);
    ...
    @Override
    public Collection<String> selectAllCompanyPerms() {
        List<String> perms = baseMapper.selectAllCompanyPerms();
        Set<String> permsSet = new HashSet<>();
        for (String perm : perms) {
            if (StringUtils.isNotEmpty(perm)) {
                permsSet.addAll(StringUtils.splitList(perm.trim()));
    ...
    @Override
    public List<SysMenu> list(QueryWrapper<SysMenu> queryWrapper) {
