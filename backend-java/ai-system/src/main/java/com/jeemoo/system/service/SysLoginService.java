...
import com.jeemoo.common.enums.DeviceType;
import com.jeemoo.common.enums.LoginType;
import com.jeemoo.common.enums.UserStatus;
...
import com.jeemoo.system.mapper.SysUserMapper;
...
public class SysLoginService {
    ...
    public String login(String username, String password, String code, String uuid) {
        ...
        if (!user.getUserType().equals(UserType.SYS_USER.getUserType())) {
    ...
    public String login(String username, String password, String userType) {
        return login(username, password, userType, DeviceType.PC);
    }

    public String login(String username, String password, String userType, DeviceType deviceType) {
        boolean captchaEnabled = configService.selectCaptchaEnabled();
    ...
    public String smsLogin(String phonenumber, String smsCode, String userType) {
        // 通过手机号查找用户
        SysUser user = loadUserByUsername(phonenumber);
        if (!user.getUserType().equals(userType)) {
     ...
     */
     public boolean validateSmsCode(String phonenumber, String smsCode) {
        String code = RedisUtils.getCacheObject(CacheConstants.SMS_CODE_KEY + phonenumber);
    ...
    private void checkLogin(LoginType loginType, String username, Supplier<Boolean> supplier) {
        String errorKey = CacheConstants.ERR_CNT_KEY + username;
    ...
    public String loginByUserId(Long userId, DeviceType deviceType) {
        // 框架登录不限制从什么表查询 只要最终构建出 LoginUser 即可
        SysUser user = loadUserByUserId(userId);
        // 此处可根据登录用户的数据不同 自行创建 loginUser 属性不够用继承扩展就行了
        LoginUser loginUser = buildLoginUser(user);
        // 生成token
        LoginHelper.loginByDevice(loginUser, deviceType);
