...
import com.jeemoo.common.constant.CacheNames;
import com.jeemoo.common.constant.UserConstants;
import com.jeemoo.common.core.domain.PageQuery;
import com.jeemoo.common.core.domain.entity.SysDept;
...
import com.jeemoo.common.core.service.UserService;
import com.jeemoo.common.exception.ServiceException;
import com.jeemoo.common.exception.biz.UserLimitExceededException;
import com.jeemoo.common.helper.DataBaseHelper;
...
import com.jeemoo.system.domain.SysPost;
import com.jeemoo.system.domain.SysUserPost;
...
public class SysUserServiceImpl implements ISysUserService, UserService {
    ...
    private Wrapper<SysUser> buildQueryWrapper(SysUser user) {
        ...
        wrapper.eq("u.del_flag", UserConstants.USER_NORMAL)
                ...
                .like(ObjectUtil.isNotNull(user.getCompanyName()), "c.company_name", user.getCompanyName())
                .eq(ObjectUtil.isNotNull(user.getUserType()),"u.user_type",user.getUserType())
                .eq(ObjectUtil.isNotNull(user.getCompanyId()) && user.getCompanyId() != 0L, "u.company_id", user.getCompanyId())
    ...
    public int insertUser(SysUser user) {
        ...
        // 新增用户与角色管理
        insertUserRole(user);
        companyMapper.addUserCount(user.getCompanyId());
        return rows;
    }
    ...
    public int updateUserStatus(SysUser user) {
        return baseMapper.updateUserStatus(user);
    }
    ...
    public int deleteUserByIds(Long[] userIds) {
        ...
        companyMapper.subUsersCount(userIds);
        return baseMapper.deleteBatchIds(ids);
    }

    @Override
    public void setCompany(SysUser user, AiCompanyBo bo) {
        baseMapper.setCompany(user.getUserId(), bo);
    }

    @Override
    public TableDataInfo<SysUser> selectPageByUser(SysUser user, LoginUser loginUser, PageQuery pageQuery) {
        user.setCompanyId(loginUser.getCompanyId());
        Page<SysUser> page = baseMapper.selectPageByUser(pageQuery.build(), loginUser, this.buildQueryWrapper(user));
        return TableDataInfo.build(page);
    }

    @Override
    public void checkUserLimit(LoginUser loginUser) {
        AiCompany company = companyMapper.selectById(loginUser.getCompanyId());
        long userCount = baseMapper.selectCount(new QueryWrapper<SysUser>().eq("company_id",loginUser.getCompanyId()));
        if (userCount >= company.getUserCountMax()) {
            throw new UserLimitExceededException();
        }
    }

    @Override
    public void setIsAlert(Long userId, Integer isAlert) {
        baseMapper.setIsAlert(userId,isAlert);
    }
